...
func ScanBundle(
	...
	// println(fmt.Sprintf("@@scanAllDependencies %v", extras))
	s.scanAllDependencies(extras)
...
func (s *scanner) preprocessInjectedFiles() {
	...
	for _, absPath := range s.options.InjectAbsPaths {
		...
		s.maybeParseFile(*resolveResult, prettyPath, nil, logger.Range{}, nil, inputKindNormal, channel, make([]string, 0))
...
func (s *scanner) addEntryPoints(entryPoints []EntryPoint) []graph.EntryPoint {
	...
	if stdin := s.options.Stdin; stdin != nil {
		...
		sourceIndex := s.maybeParseFile(resolveResult, s.res.PrettyPath(stdinPath), nil, logger.Range{}, nil, inputKindStdin, nil, make([]string, 0))
	...
	for i, resolveResult := range entryPointResolveResults {
		if resolveResult != nil {
			...
			sourceIndex := s.maybeParseFile(*resolveResult, prettyPath, nil, logger.Range{}, resolveResult.PluginData, inputKindEntryPoint, nil, make([]string, 0))
...
func (s *scanner) scanAllDependencies(extras []string) {
			...
			for importRecordIndex := range records {
				...
				if !resolveResult.IsExternal {
					...
					sourceIndex := s.maybeParseFile(*resolveResult, s.res.PrettyPath(path),
						&result.file.inputFile.Source, record.Range, resolveResult.PluginData, inputKindNormal, nil, extras)
