...
function flagsForBuildOptions(
  ...
  let spinxHotReact =
    getFlag(options, keys, "spinxHotReact", mustBeBoolean) ?? false;
  ...
  if (spinxAssetBaseUrl)
    flags.push(`--spinx-asset-base-url=${spinxAssetBaseUrl}`);
...
export function createChannel(streamIn: StreamIn): StreamOut {
  type PluginCallback = (
    request: protocol.OnResolveRequest | protocol.OnLoadRequest
  ...
  let handleIncomingPacket = (bytes: Uint8Array): void => {
    ...
    if (isFirstPacket) {
      ...
      if (binaryVersion !== ESBUILD_VERSION) {
        throw new Error(
          `Cannot start service: Host version "${ESBUILD_VERSION}" does not match binary version ${JSON.stringify(
            binaryVersion
          )}`
        );
      }
    ...
    for (let item of plugins) {
      ...
      setup({
        ...
        onResolve(options, callback) {
          ...
          if (filter == null)
            throw new Error(
              `[${plugin.name}] onResolve() call is missing a filter`
            );
        ...
        onLoad(options, callback) {
          ...
          if (filter == null)
            throw new Error(
              `[${plugin.name}] onLoad() call is missing a filter`
            );
          let id = nextCallbackID++;
            ...
            try {
              ...
              if (result != null) {
                if (typeof result !== "object")
                  throw new Error(
                    `Expected onResolve() callback in plugin ${JSON.stringify(
                      name
                    )} to return an object`
                  );
                let keys: OptionKeys = {};
              ...
              return {
                id,
                errors: [
                  extractErrorMessageV8(e, streamIn, stash, note && note()),
                ],
              };
            }
          }
          ...
          for (let id of request.ids) {
            try {
              ...
              if (result != null) {
                if (typeof result !== "object")
                  throw new Error(
                    `Expected onLoad() callback in plugin ${JSON.stringify(
                      name
                    )} to return an object`
                  );
                let keys: OptionKeys = {};
      ...
      buildOrServe(
        ...
        try {
          ...
          if (plugins && plugins.length > 0) {
            [requestPlugins, pluginRefs] = handlePlugins(
              options,
              plugins,
              key,
              details
          ...
          let serve =
            serveOptions && buildServeData(refs, serveOptions, request);
          ...
          let copyResponseToResult = (
            ...
            if (response.outputFiles)
              result.outputFiles = response!.outputFiles.map(
                convertOutputFiles
              );
          ...
          let buildResponseToResult = (
            ...
            if (errors.length > 0)
              return callback(
                failureErrorWithLog("Build failed", errors, warnings),
                null
              );
              ...
              if (!stop) {
                ...
                stop = () => {
                  ...
                  sendRequest<protocol.WatchStopRequest, null>(
                    refs,
                    { command: "watch-stop", watchID: response!.watchID! },
                    () => {
                      // We don't care about the result
                    }
                  );
                ...
                if (watch && watch.onRebuild) {
                  watchCallbacks.set(
                    ...
                    (serviceStopError, watchResponse) => {
                      if (serviceStopError)
                        return watch!.onRebuild!(serviceStopError as any, null);
                      let errors = replaceDetailsInMessages(
          ...
          if (incremental && streamIn.isSync)
            throw new Error(
              `Cannot use "incremental" with a synchronous build`
            );
        ...
        let start = (inputPath: string | null) => {
          try {
            ...
            sendRequest(refs, { command: "error", flags, error }, () => {
              error.detail = details.load(error.detail);
              callback(
                failureErrorWithLog("Transform failed", [error], []),
                null
              );
            });
          }
